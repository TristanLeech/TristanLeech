<img align="right" src="https://komarev.com/ghpvc/?username=TristanLeech&label=Profile%20views&color=0a456b&style=flat" alt="ViewCounter" />

![profileBanner](https://github.com/user-attachments/assets/f8a4861a-7c11-43d8-9450-d8ffa9430448)

<h1 align="right">
Hi, I'm Tristan,
</h1>
<h2 align="right">
Get to know me through my work
</h2>

<p align="center">
  <img src="https://img.shields.io/badge/Frontend%20Developer-007ACC?logo=codecrafters&logoColor=white" alt="Frontend Developer" />
  <img src="https://img.shields.io/badge/UX%20Designer-D0B0FF?logo=github&logoColor=black" alt="UX Designer" />
  <img src="https://img.shields.io/badge/HTML5-grey?logo=html5&logoColor=white" alt="HTML" />
  <img src="https://img.shields.io/badge/JavaScript-F0DB4F?logo=javascript&logoColor=323330" alt="JavaScript" />
  <img src="https://img.shields.io/badge/CSS-FFFFFF?logo=css&logoColor=grey" alt="CSS" alt="CSS"/>
  <img src="https://img.shields.io/badge/MongoDB-4EA94B?logo=mongodb&logoColor=white" alt="mongo"/>
  <img src="https://img.shields.io/badge/Express-000000?logo=express&logoColor=white" alt="JavaScript"express/>
  <img src="https://img.shields.io/badge/React-61DBFB?logo=react&logoColor=black" alt="React" />
  <img src="https://img.shields.io/badge/Node.js-339933?logo=node.js&logoColor=white" alt="Node.js" /> 
  <img src="https://img.shields.io/badge/Heroku-6762a6?logo=heroku&logoColor=c9c3e6 " alt="Heroku" />
  <img src="https://img.shields.io/badge/Figma-F24E1E?logo=figma&logoColor=white" alt="Figma" />
  <img src="https://img.shields.io/badge/Loves&nbsp;Birds-eb9928?logo=padlet&logoColor=white" alt="bird" />
  <!-- <img src="https://img.shields.io/badge/Musician-a280ff?logo=applemusic&link=(https://instagram.com/cerealsauceband)" alt="bandLink" /> -->
</p>

<div align="center">

<br />
  
  # Bio
  
  I'm a maker, studying Industrial Design and Web Development.
  You can find me making art, music, or food, reading, and gaming
  I have four brothers, and I've been raising birds since I was three

  <br />
  
  ---
  
  <br />

  # Skills
  
  **I'm profficient with:**
  
- **JavaScript** (React, Node.js)
- **HTML**
- **CSS**
- **MySQL**
- **MongoDB**

  <br />
  
  ---

  <br />
  
</div>

# Portfolio Projects

## [Prospero](https://github.com/TristanLeech/prospero)
<img height="250" alt="Screenshot 2025-11-16 225523" src="https://github.com/user-attachments/assets/cae4a4d6-094e-466b-a57d-c56bea50ff78" />


**Prospero is an organisational tool intended for neurodivergent audiences**

The website was designed to allow users to create a calendar and populate it with events they wish to keep track of.

The project was made using HTML, CSS, MySQL, and Node.js, with a React framework

[View Repository](https://github.com/TristanLeech/prospero)

<br />

--- 
<br />

## [Vinoir](https://github.com/corbyn-jpg/summative-vinoir)
<img width="821" height="250" alt="vinoirBanner" src="https://github.com/user-attachments/assets/5ed2f190-9bfc-49fd-b29f-cb80ecf87f7f" />

**Vinoir is an E-Commerce Website for a fictional luxury perfume store**

Group Authorship: Corbyn Robinson, Mario Ojo, Tristan Leech

The site allows users to create an account, browse perfumes, and add them to their cart. Account creation includes an additional visual password, and users can filter perfumes by different features.

The project was made using HTML, CSS, MongoDB, Express, Node.js, and Material-UI, with a React framework

[View Repository](https://github.com/corbyn-jpg/summative-vinoir)

<br />

--- 

<br />

## [Cocktail Comparisons](https://github.com/TristanLeech/cocktail-database)

<img width="775" height="269" alt="cocktailBanner" src="https://github.com/user-attachments/assets/c5bb3aad-3af3-4051-9dc6-015d8e6d7cc3" />

**This website displays and compares various drinks and their ingredients**

This site is fairly basic, allowing users to view cocktail recipes, and visually see what they are comprised of.

The project was made using HTML, CSS, with a React framework, and the use of [TheCocktailDB](https://www.thecocktaildb.com/api.php)

[View Repository](https://github.com/TristanLeech/cocktail-database)

--- 

<!--
bio
skills
tech stack badges
portfolion project repos
github repo stats
contact info
-->
